...
public class CreateAuthorDtoValidator : AbstractValidator<CreateAuthorDto>
{
    public CreateAuthorDtoValidator()
    {
        RuleFor(x => x.AuthorId)
            ...
            .MaximumLength(11).WithMessage("Author ID must not exceed 11 characters")
            .Matches(@"^\d{3}-\d{2}-\d{4}$").WithMessage("Author ID must be in format ###-##-####");
